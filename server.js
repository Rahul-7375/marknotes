...
const db = require('./db'); // This is now a MySQL pool
...
const parseNote = (n) => {
  ...
  return {
    ...n,
    // MySQL handles JSON columns automatically, but just in case:
...
api.get('/notes',
  ...
  async (req, res) => {
    try {
      ...
      if (tag) { 
        // Using MySQL JSON_CONTAINS for robust tag matching
...
api.use((req, res) => err(res, 'Route not found', 404));
...
if (fs.existsSync(CLIENT_BUILD)) {
  ...
  app.get('/', (_, res) => res.send(
    `<h2>API is running on port ${PORT} (MySQL Mode)</h2>
     <p>Ensure your MySQL server is running and database "${process.env.DB_NAME || 'marknotes'}" exists.</p>`
  ));
}

// ── Start ─────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`\n✅  MarkNotes (MySQL) running → http://localhost:${PORT}\n`);
});
